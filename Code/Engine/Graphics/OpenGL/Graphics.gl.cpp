...
bool Engine::Graphics::CreateVertexArray()
{
	bool wereThereErrors = false;
	...
	{
		// We are drawing a square
		const unsigned int vertexCount = 4; // EAE6320_TODO;	// What is the minimum number of vertices a square needs (so that no data is duplicated)?
	...
	{
		// We are drawing a square
		const unsigned int triangleCount = 2;  // EAE6320_TODO;	// How many triangles does a square have?
		...
		Mesh::setVertexID(s_vertexArrayId); //Setting the Vertex Array ID to the mesh component
	}
