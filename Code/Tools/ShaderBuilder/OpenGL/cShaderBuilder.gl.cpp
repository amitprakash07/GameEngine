...
#include "../../../Externals/Mcpp/Includes.h"
	...
	bool SaveShaderSource( const char* const i_path, const std::string& i_shader )
	{
		...
		// Open the file
		{
			...
			if ( fileHandle == INVALID_HANDLE_VALUE )
			{
				...
				errorMessage << "Windows failed to open the target shader file for writing: " << WindowsUtil::GetLastWindowsError();
				errorMessage << "in file" << i_path;
		...
		// Write the modified shader source
		{
			...
			if ( WriteFile( fileHandle, i_shader.c_str(), bytesToWriteCount, &bytesWrittenCount, writeSynchronously ) != FALSE )
			{
				if ( bytesWrittenCount != bytesToWriteCount )
				{
					...
					errorMessage << "Windows was supposed to write out " << bytesToWriteCount << " bytes for the target shader, "
						"but only actually wrote " << bytesWrittenCount<< "in file "<<i_path;
					WindowsUtil::Print(errorMessage.str().c_str());
					goto OnExit;
				}
			}
			else
			{
				wereThereErrors = true;
				std::stringstream errorMessage;
				errorMessage << "Windows failed to write the target shader: " << WindowsUtil::GetLastWindowsError()<< "in file "<<i_path;
				WindowsUtil::Print(errorMessage.str().c_str());
				goto OnExit;
			}
		}
	...
	OnExit:
		...
		if ( fileHandle != INVALID_HANDLE_VALUE )
		{
			if ( CloseHandle( fileHandle ) == FALSE )
			{
				wereThereErrors = true;
				std::stringstream errorMessage;
				errorMessage << "Windows failed to close the target shader file's handle: " << WindowsUtil::GetLastWindowsError()
					<<"in File "<<i_path;
				WindowsUtil::Print(errorMessage.str().c_str());
				goto OnExit;
			}
