...
bool Tools::AssetBuilder::cTextureBuilder::Build( const std::vector<std::string>& )
{
	...
	D3DXIMAGE_INFO imageInfo_source;
	{
		...
		{
			wereThereErrors = true;
			errormessage << "DirectX failed to get image info for the source image";
			WindowsUtil::Print(errormessage.str().c_str() , m_path_source );
			goto OnExit;
		}
	}
	...
	// Save the texture
	{
		...
		{
			wereThereErrors = true;
			errormessage << "DirectX failed to save the texture";
			WindowsUtil::Print(errormessage.str().c_str(), m_path_target);
			goto OnExit;
		}
	}
...
namespace
{
	bool Initialize( const char* i_path_source )
	{
		...
		// Create the D3D9 interface
		{
			...
			{
				errormessage << "DirectX failed to create a Direct3D9 interface";
				WindowsUtil::Print(errormessage.str().c_str() , i_path_source );
				return false;
			}
		}
