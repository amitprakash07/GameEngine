...
#include "../../../Code/Engine/Core/Octree/Octree.h"
#include "../../Engine/Core/Octree/AABBV.h"
...
MStatus EAE_Engine::Tools::cMayaOctreeExporter::writer( const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode )
{
	MStatus status;
...
namespace
{
		...
		const std::vector<sMaterialInfo>& i_materialInfo )
	{
		...
		if ( fout.is_open() )
		{
			Engine::Math::Vector3 minPos(FLT_MAX, FLT_MAX, FLT_MAX);
			Engine::Math::Vector3 maxPos(-FLT_MAX, -FLT_MAX, -FLT_MAX);
			for (std::vector<sVertex_maya>::const_iterator it = i_vertexBuffer.begin(); it != i_vertexBuffer.end(); ++it)
			{
				...
				if (vertex.z < minPos.z)
					minPos.z = vertex.z;
				if (vertex.x > maxPos.z)
					maxPos.z = vertex.z;
			}
			// Open table
			Engine::Physics::Octree completeOctree;
			...
			Engine::Physics::OctreeNode* pLeaveNodes = completeOctree.GetNodesInLevel(leafLevelIndex);
			...
			for (uint32_t leafIndex = 0; leafIndex < coutOfLeaves; ++leafIndex)
			{
				for (size_t index = 0; index < i_indexBuffer.size(); index += 3)
				{
					...
					uint32_t index0 = static_cast<uint32_t>(i_indexBuffer[index + 0]);
					uint32_t index1 = static_cast<uint32_t>(i_indexBuffer[index + 1]);
					uint32_t index2 = static_cast<uint32_t>(i_indexBuffer[index + 2]);
					Engine::Physics::TriangleIndex triangle = { index0, index1, index2 };
					// get the 3 vertices of the trianlge
					sVertex_maya vertex_maya0 = i_vertexBuffer[index0];
					Engine::Math::Vector3 vertex0(vertex_maya0.x, vertex_maya0.y, vertex_maya0.z);
					sVertex_maya vertex_maya1 = i_vertexBuffer[index1];
					Engine::Math::Vector3 vertex1(vertex_maya1.x, vertex_maya1.y, vertex_maya1.z);
					sVertex_maya vertex_maya2 = i_vertexBuffer[index2];
					Engine::Math::Vector3 vertex2(vertex_maya2.x, vertex_maya2.y, vertex_maya2.z);
					// create the AABB for this leaf
					Engine::Physics::AABBV1 aabb;
					aabb._min = pLeaveNodes[leafIndex].GetMin();
					aabb._max = pLeaveNodes[leafIndex].GetMax();
					// do the test
					bool contains = Engine::Physics::TestTriangleAABB(vertex0, vertex1, vertex2, aabb);
					if(contains)
					{
						pLeaveNodes[leafIndex].triangles.push_back(triangle);
			...
			uint32_t sizeOfBuffer = sizeof(uint32_t) + sizeof(uint32_t) + sizeof(Engine::Math::Vector3) + sizeof(Engine::Math::Vector3) + 
				+countOfTrianglesAdded * sizeof(uint32_t) + countOfTrianglesAdded * sizeof(Engine::Physics::TriangleIndex) + 1;
			Engine::Math::Vector3 min = completeOctree.GetMin();
			Engine::Math::Vector3 max = completeOctree.GetMax();
			Engine::Physics::OctreeNode* pNodes = completeOctree.GetNodes();
			char* pBuffer = new char[sizeOfBuffer];
			memset(reinterpret_cast<uint8_t*>(pBuffer), sizeOfBuffer, 0);
			{
				uint32_t offset = 0;
				uint32_t level = completeOctree.Level();
				//EAE_Engine::CopyMem((uint8_t*)&level, (uint8_t*)(pBuffer)+offset, sizeof(uint32_t));
				memcpy(reinterpret_cast<uint8_t*>(&level), reinterpret_cast<uint8_t*>(pBuffer) + offset, sizeof(uint32_t));
				offset += sizeof(uint32_t);
				//EAE_Engine::CopyMem((uint8_t*)&nodesCount, (uint8_t*)(pBuffer)+offset, sizeof(uint32_t));
				memcpy(reinterpret_cast<uint8_t*>(&nodesCount), reinterpret_cast<uint8_t*>(pBuffer) + offset, sizeof(uint32_t));
				offset += sizeof(uint32_t);
				//EAE_Engine::CopyMem((uint8_t*)&min, (uint8_t*)(pBuffer) + offset, sizeof(EAE_Engine::Math::Vector3));
				memcpy(reinterpret_cast<uint8_t*>(&min), reinterpret_cast<uint8_t*>(pBuffer) + offset, sizeof(Engine::Math::Vector3));
				offset += sizeof(Engine::Math::Vector3);
				//EAE_Engine::CopyMem((uint8_t*)&max, (uint8_t*)(pBuffer)+offset, sizeof(Engine::Math::Vector3));
				memcpy(reinterpret_cast<uint8_t*>(&max), reinterpret_cast<uint8_t*>(pBuffer) + offset, sizeof(Engine::Math::Vector3));
				offset += sizeof(Engine::Math::Vector3);
				// Now let's fill in the trianlges information
				Engine::Physics::OctreeNode* pLeaves = completeOctree.GetNodesInLevel(leafLevelIndex);
				...
				for (uint32_t leafIndex = 0; leafIndex < countOfLeaves; ++leafIndex)
				{
					...
					uint32_t countOfTrianlges = static_cast<uint32_t>(pLeaves[leafIndex].triangles.size());
					//EAE_Engine::CopyMem((uint8_t*)&countOfTrianlges, (uint8_t*)pBuffer+offset, sizeof(uint32_t));
					memcpy(reinterpret_cast<uint8_t*>(&countOfTrianlges), reinterpret_cast<uint8_t*>(pBuffer) + offset, sizeof(uint32_t));
					offset += sizeof(uint32_t);
					if (countOfTrianlges > 0)
					{
						// record trianlges in this node
						uint32_t bufferSizeForTrianlge = sizeof(Engine::Physics::TriangleIndex) * countOfTrianlges;
						//EAE_Engine::CopyMem((uint8_t*)&(pLeaves[leafIndex]._triangles[0]), (uint8_t*)pBuffer + offset, bufferSizeForTrianlge);
						memcpy(reinterpret_cast<uint8_t*>(&(pLeaves[leafIndex].triangles[0])), reinterpret_cast<uint8_t*>(pBuffer) + offset, bufferSizeForTrianlge);
